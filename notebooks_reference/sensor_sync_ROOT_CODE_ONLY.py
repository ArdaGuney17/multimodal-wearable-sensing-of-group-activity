...
def load_xsens(filepath):
    """Load Xsens DOT CSV.
    SampleTimeFine is in microseconds (relative hardware counter)."""
    df = pd.read_csv(filepath, skipinitialspace=True)  # ← fixes space-after-comma issue
...
for name in PARTICIPANTS:
    ...
    print(f'{name_cap:10s}  Xsens peak: {xs_peak:.2f}s   OE peak: {oe_peak:.2f}s   '
...
colors     = ['steelblue', 'tomato', 'seagreen']
linestyles = ['-', '--', '-.']   # different style per participant
ax = axes[0]

for name, color, ls in zip(PARTICIPANTS, colors, linestyles):
    df  = aligned[name]
    mag = np.sqrt(df['xs_acc_x']**2 + df['xs_acc_y']**2 + df['xs_acc_z']**2)
    mask = (df['time_sec'] >= elan_sync_t - 90) & (df['time_sec'] <= elan_sync_t + 30)
    ax.plot(df.loc[mask, 'time_sec'], mag[mask],
            label=name.capitalize(), color=color,
            lw=1.2, alpha=0.75, linestyle=ls)   # ← alpha + different linestyles

ax.axvline(elan_sync_t, color='black', lw=2.5, ls='--',
           label=f'ELAN sync ({elan_sync_t:.1f}s)')
ax.set_title('Step 1 Verification — Xsens acc magnitude on ELAN timeline\n'
             'All peaks should align at the black dashed line', fontweight='bold')
...
for name, color, ls in zip(PARTICIPANTS, colors, linestyles):
    df  = aligned[name]
    mag = np.sqrt(df['oe_acc_x']**2 + df['oe_acc_y']**2 + df['oe_acc_z']**2)
    mask = (df['time_sec'] >= elan_sync_t - 90) & (df['time_sec'] <= elan_sync_t + 30)
    ax2.plot(df.loc[mask, 'time_sec'], mag[mask],
             label=name.capitalize(), color=color,
             lw=1.2, alpha=0.75, linestyle=ls)

ax2.axvline(elan_sync_t, color='black', lw=2.5, ls='--')
ax2.set_title('OpenEarable acc magnitude (cross-check)', fontweight='bold')
ax2.set_xlabel('ELAN time (s)')
ax2.set_ylabel('Acc magnitude (m/s²)')
ax2.legend()
