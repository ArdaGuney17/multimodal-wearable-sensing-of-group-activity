...
| Windowing & model-ready assembly | Ch.4 §4.7–4.9 (Table 4.4) | `PRE_PROCESSING/Global_Cleaning_Before_Model.ipynb` (label-text normalization via `GLOBAL_LABEL_REPLACEMENTS`, trim-to-labeled-range, and — its own final cell — the Optitrack participant-identity fix via `PARTICIPANT_POSITION_MAP`; **not actually windowed** — output is still per-sample) → `ALL_MODEL_READY_FILES_IDENTITY_FIXED/group_{g}_{sensor}_model_ready.csv` (outputs already exist — treat as regression fixtures) | `src/preprocessing/global_cleaning.py` | ⬜ not yet ported (analysis complete — see `notebooks_reference/Global_Cleaning_Before_Model_ANALYSIS.md`) |
